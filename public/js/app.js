// app.js
// L.1-55   helpers (el, fileToBase64, apiPost, setStatus/Error, buildParts)
// L.57-115 filas de fuentes (URL / PDF / Pegar texto)
// L.117-185 PASO 1: analizar → temas
// L.187-270 PASO 2: generar + redistribuirRespuestas
// L.272-320 PASO 3: descarga individual + ZIP
// L.322-340 init
"use strict";
...
function el(tag, attrs = {}, children = []) {
  ...
  [].concat(children).forEach(c => c != null &&
  ...
  sel.addEventListener("change", () => {
    ...
    if (sel.value === "url") {
      ...
      const st = el("span", { class: "muted", style: "display:block;margin-top:4px" });
        ...
        st.textContent = f.size > 6 * 1024 * 1024
          ? "⚠️ PDF mayor a 6 MB, puede tardar." : `✓ ${f.name}`;
        sourcesState[index] = {
          type: "pdf", filename: f.name,
          mimeType: "application/pdf", base64: await fileToBase64(f),
        };
      });
...
async function analizarFuentes() {
  ...
  const advertencias = [];
  try {
    // URLs: fetch server-side (Vercel), sin CORS ni intervención del docente
    ...
    if (!textsCache.length && !pdfsCache.length) {
      throw new Error(advertencias.length
        ? advertencias.join(" — ")
        : "Sin contenido válido. Completá al menos una fuente.");
    }
...
Respondé ÚNICAMENTE con JSON: { "topics": ["tema 1", ...] }`,
    ...
    const topics = (extractJson(text).topics || [])
...
const ESTILOS = {
  multiple:    `Cada pregunta es DIRECTA con 4 opciones (A-D).`,
  comprension: `Cada pregunta incluye un párrafo breve del material (precedido de "Leé el siguiente texto:"), seguido de una pregunta de comprensión con 4 opciones.`,
  completar:   `Cada pregunta es una oración con un espacio en blanco (___) que se completa con la opción correcta. Las 4 opciones son palabras o frases cortas.`,
  vof:         `Cada pregunta presenta 4 afirmaciones sobre el tema. Solo UNA es verdadera. El enunciado es: "¿Cuál de las siguientes afirmaciones es VERDADERA?"`,
  caso:        `Cada pregunta describe una situación o caso práctico y pregunta cómo se aplica el concepto, cuál es la causa o qué decisión corresponde. 4 opciones.`,
  mixto:       `Usá una mezcla equilibrada: algunas preguntas directas, algunas de comprensión lectora, algunas de completar la frase, algunas de verdadero/falso y algunas de caso práctico.`,
};
...
async function generarExamen() {
  ...
  if (!textsCache.length && !pdfsCache.length) {
    setError("generar-error", "Analizá el contenido primero."); return;
  }
  ...
  const estiloInstruccion = ESTILOS[getEstiloSeleccionado()] || ESTILOS.multiple;
  ...
  try {
    for (let v = 1; v <= TOTAL_V; v++) {
      ...
      const text = await geminiGenerate(
        `Sos un profesor. Armá la VERSIÓN ${v} de ${TOTAL_V} de un examen de opción múltiple
basado en el material adjunto.
...
- Distribuí la posición correcta equilibradamente (aprox. igual en 0, 1, 2 y 3).
- Opciones incorrectas plausibles.
- Cada pregunta debe ser AUTOCONTENIDA: no uses frases como "según el texto" ni
  referencias al material. Si necesitás un párrafo de contexto, incluiló dentro del campo "q".
- Usá SOLO caracteres latinos. Escribí palabras de otros alfabetos en transcripción
  fonética (ej: "mathematikos" en vez de la letra griega).
...
[{"q":"...","opts":["a","b","c","d"],"ans":2},...]`,
      ...
      temas[String(v)] = redistribuirRespuestas(
      ...
      if (temas[String(v)].length < PREGUNTAS)
        advertencias.push(`Versión ${v}: ${temas[String(v)].length}/${PREGUNTAS} preguntas generadas.`);
...
function mostrarDescarga(temas, advertencias) {
  ...
  dl.appendChild(el("button", {
    class: "btn", onclick: () => descargarZip(temas),
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // Cargar la API key desde Vercel (env var, nunca expuesta en el código)
  try {
    const data = await (await fetch("/api/get-key")).json();
    if (data.key) window._geminiKey = data.key;
  } catch (_) {}

  // Selector de estilos de preguntas
